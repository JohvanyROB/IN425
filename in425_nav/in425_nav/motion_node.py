...
class Motion(Node):
    def __init__(self):
        ...
        self.create_subscription(Path, "/path", self.plannerCb, 1)  #Subscriber to get the path computed by the RRT-Connect node
    ...
    def plannerCb(self, msg):
        """ Callback method to get incoming path computed by the RRT-Connect node """
        #DO NOT TOUCH
    ...
    def run(self):
        ...
        if not self.reached and self.goal_received:
            ...
            #Add your navigation strategy here (self.path contains the computed path)
            #You should define the adequate self.linear and self.angular attributes to make the robot follow the path
            #Call the method self.send_velocities() once these attributes have been set correctly in order to publish the velocity commands
